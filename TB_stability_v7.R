...
#for Mac
setwd("/Volumes/Morgan_SSD/R work/TB_stability")
search()
...
#####################################################################################################
############# Make combined fig of rich, synch, turn, tot cov without bromes ##########
title1 <- expression(paste(italic("B. arvensis")))
title2 <- expression(paste(italic("B. tectorum")))

#synchrony
BRARsynch_fig_nobrome <- ggplot(data = BRARsynch_nobrome2, aes(x = avgBRAR, y = synch)) + 
  ylim(0, 0.7) + 
  geom_point(size = 3) + 
  labs(x = "", y = "Synchrony", title = title1) +
...
noBRAR_turn_cov2 <- ggplot(data = noBRAR_turn_cov, aes(x = avgBRAR, y = total)) + 
  ...
  labs(x = "", y = "Species Turnover", title = title1) +
  theme(plot.title = element_text(vjust = - 7, hjust = 0.07, size = 16), axis.text.x = element_blank()) + 
  theme(panel.border = element_rect(linewidth = 2))
...
BRARrichstab3_nobrome <- ggplot(data = BRARrichstab2_nobrome, aes(x = avgBRAR, y = stability)) + 
  geom_point(size = 3) + 
  ylim(0, 30) +
  labs(x = "", y = "Richness Stability", title = title1) +
  theme(plot.title = element_text(vjust = - 7, hjust = 0.07, size = 16), axis.text.x = element_blank()) + 
  theme(panel.border = element_rect(linewidth = 2))
...
noBRAR_stab3 <- ggplot(data = BRARcov_only3, aes(x = avgBRAR, y = stability)) + 
  ylim(0, 30) + 
  geom_point(size = 3) + 
  labs(x = "Relative % Cover", y = "Cover Stability", title = title1) +
...
noBRTE_stab3 <- ggplot(data = BRTEcov_only3, aes(x = avgBRTE, y = stability)) + 
  ylim(0, 20) +
  geom_point(size = 3) + 
  labs(x = "Relative % Cover", y = "", title = title2) +
  theme(plot.title = element_text(vjust = - 7, hjust = 0.07, size = 16)) + 
  theme(panel.border = element_rect(linewidth = 2))


BRARsynch_fig_nobrome + BRTEsynch_fig_nobrome + noBRAR_turn_cov2 + noBRTE_turn_cov2 + 
  BRARrichstab3_nobrome + BRTErichstab3_nobrome + noBRAR_stab3 + noBRTE_stab3 + plot_layout(ncol = 2)
...
BRARc4_fig <- ggplot(data = BRARsp_c4_stab, aes(x = avgBRAR, y = stability)) +
  ...
  ylim(0, 6) +
  geom_smooth(method = "lm", se = FALSE) + 
  labs(x = "", y = "C4 Grass Stability", title = title1) +
  theme(plot.title = element_text(vjust = - 7, hjust = 0.07, size = 16), axis.text.x = element_blank()) + 
  theme(panel.border = element_rect(linewidth = 2))
...
BRARc3_nobrome_fig <- ggplot(data = BRARsp_c3_nobrome_stab, aes(x = avgBRAR, y = stability)) +
  geom_point(size = 3) + 
  ylim(0, 8) +
  labs(x = "", y = "C3 Grass Stability", title = title1) +
  theme(plot.title = element_text(vjust = - 7, hjust = 0.07, size = 16), axis.text.x = element_blank()) + 
  theme(panel.border = element_rect(linewidth = 2))
...
BRARf_fig <- ggplot(data = BRARsp_f_stab, aes(x = avgBRAR, y = stability)) +
  ...
  ylim(0, 6) +
  geom_smooth(method = "lm", se = FALSE) + 
  labs(x = "Relative % Cover", y = "Forb Stability", title = title1) +
  theme(plot.title = element_text(vjust = - 7, hjust = 0.07, size = 16)) + 
  theme(panel.border = element_rect(linewidth = 2))
...
##################################################################
######### Moderator analyses with avg richness ##########
#average richness 
BRARavgrich <- BRARcomm_struct_nobrome %>%
  group_by(plot, invasion_percent, gradient_number) %>%
  summarise(avgrich = mean(richness)) %>%
  ungroup()
...
BRAR_med_rich <- BRARcover %>%
  full_join(BRARavgrich) %>%
  full_join(BRARsynch_nobrome2) %>%
  full_join(noBRAR_turn2) %>%
  full_join(noBRAR_stab2) %>%
  full_join(BRARsp_c4_stab2) %>%
...
BRAR_med_c4_rich <- BRAR_med_rich %>%
  filter(log_c4 != "NA")
...
#average richness 
BRTEavgrich <- BRTEcomm_struct_nobrome %>%
  group_by(plot, invasion_percent, gradient_number) %>%
  summarise(avgrich = mean(richness)) %>%
  ungroup()
...
BRTE_med_rich <- BRTEcover %>%
  full_join(BRTEavgrich) %>%
  full_join(BRTEsynch_nobrome2) %>%
  full_join(noBRTE_turn2) %>%
  full_join(noBRTE_stab2) %>%
  full_join(BRTEsp_c4_stab2) %>%
...
BRTE_med_c4_rich <- BRTE_med_rich %>%
  filter(log_c4 != "Inf" & log_c4 != "NA")
BRTE_med_f_rich <- BRTE_med_rich %>%
  filter(log_f != "Inf" & log_f != "NA")
...
#est variables
#BRAR
BRAR <- BRAR_med_rich$avgBRAR
BRAR_rich <- BRAR_med_rich$avgrich
BRAR_synch <- BRAR_med_rich$synch
BRAR_turn <- BRAR_med_rich$turn
BRAR_cov <- BRAR_med_rich$log_cov
BRAR_c3 <- BRAR_med_rich$log_c3
BRAR_f <- BRAR_med_rich$log_f
BRAR2 <- BRAR_med_c4_rich$avgBRAR
BRAR_rich2 <- BRAR_med_c4_rich$avgrich
BRAR_c4 <- BRAR_med_c4_rich$log_c4

dat1_rich <- data.frame(BRAR, BRAR_rich, BRAR_synch, BRAR_turn, BRAR_cov, BRAR_c3, BRAR_f)
dat2_rich <- data.frame(BRAR2, BRAR_rich2, BRAR_c4)


#BRTE
BRTE <- BRTE_med_rich$avgBRTE
BRTE_rich <- BRTE_med_rich$avgrich
BRTE_synch <- BRTE_med_rich$synch
BRTE_turn <- BRTE_med_rich$log_turn
BRTE_cov <- BRTE_med_rich$log_cov
BRTE_c3 <- BRTE_med_rich$log_c3
BRTE_c4 <- BRTE_med_c4_rich$log_c4
BRTE_f <- BRTE_med_f_rich$log_f
BRTE2 <- BRTE_med_rich$avgBRTE
BRTE3 <- BRTE_med_c4_rich$avgBRTE
BRTE4 <- BRTE_med_f_rich$avgBRTE
BRTE_rich3 <- BRTE_med_c4_rich$avgrich
BRTE_rich4 <- BRTE_med_f_rich$avgrich


dat3_avg <- data.frame(BRTE, BRTE_rich, BRTE_synch, BRTE_turn, BRTE_cov, BRTE_c3)
dat5_avg <- data.frame(BRTE3, BRTE_rich3, BRTE_c4)
dat6_avg <- data.frame(BRTE4, BRTE_rich4, BRTE_f)
...
#full
BRAR_synch_full <- 'BRAR_rich ~ BRAR
                        BRAR_synch ~ BRAR_rich + 0*BRAR'

BRAR_synch_full_sem <- sem(BRAR_synch_full, dat1_rich)


#part
BRAR_synch_part <- 'BRAR_rich ~ a*BRAR
                        BRAR_synch ~ c*BRAR_rich + e*BRAR
                        rich_path := c*a
                        no_path := e'

BRAR_synch_part_sem <- sem(BRAR_synch_part, dat1_rich)


#no
BRAR_synch_no <- 'BRAR_synch ~ BRAR
                  BRAR_rich ~ BRAR'
BRAR_synch_no_sem <- sem(BRAR_synch_no, dat1_rich)


AICc(BRAR_synch_full_sem, BRAR_synch_part_sem, BRAR_synch_no_sem) #AICc = 63.90221, 64.42067, 64.42067
dist(AICc(BRAR_synch_full_sem, BRAR_synch_part_sem, BRAR_synch_no_sem)[,2])
...
#correcting for 9 stability metrics, separate per brome

#without brome data included - 
correct_noBRAR <- c(0.1562, 0.9335, 0.6921, 0.01998, 0.3395, 0.4181, 0.02457, 0.2657, 0.01687)
p.adjust(correct_noBRAR, method = "bonferroni", n = 9)
#1.00000 1.00000 1.00000 0.17982 1.00000 1.00000 0.22113 1.00000 0.15183


correct_noBRTE <- c(0.153, 0.06556, 0.4246, 0.0006951, 0.7263, 0.04223, 0.8744, 0.001387, 0.06719)
p.adjust(correct_noBRTE, method = "bonferroni", n = 9)
#1.0000000 0.5900400 1.0000000 0.0062559 1.0000000 0.3800700 1.0000000 0.0124830 0.6047100



#with brome data
correct_BRAR <- c(0.1922, 0.1508, 0.3476, 0.02802, 0.6794, 0.2829)
p.adjust(correct_BRAR, method = "bonferroni", n = 6)
#1.00000 0.90480 1.00000 0.16812 1.00000 1.00000

correct_BRTE <- c(0.03639, 0.2997, 0.22, 0.001718, 0.7106, 0.1433)
p.adjust(correct_BRTE, method = "bonferroni", n = 6)
# 0.218340 1.000000 1.000000 0.010308 1.000000 0.859800
...
#Adjustments without bareground and evenness
#correcting for multiple stability metrics, separate per brome

#without brome data included - whole community metrics
correct_noBRAR2 <- c(0.1562, 0.6921, 0.01998, 0.3395)
p.adjust(correct_noBRAR2, method = "BH", n = 4)
#0.62480 1.00000 0.07992 1.00000 - doesnt change with BH 
#BH 0.3124000 0.6921000 0.0799200 0.4526667

correct_noBRTE2 <- c(0.0153, 0.4246, 0.0006951, 0.7263)
p.adjust(correct_noBRTE2, method = "BH", n = 4)
#0.0612000 1.0000000 0.0027804 1.0000000 - with BH, 0.06 is 0.03, otherwise doesnt change
#BH  0.0306000 0.5661333 0.0027804 0.7263000

#without brome data included - funct group metrics
correct_noBRAR3 <- c(0.02457, 0.2657, 0.01687)
p.adjust(correct_noBRAR3, method = "BH", n = 3)
#0.07371 0.79710 0.05061 - BH makes more significant
#BH 0.036855 0.265700 0.036855

correct_noBRTE3 <- c(0.8744, 0.001387, 0.06719)
p.adjust(correct_noBRTE3, method = "BH", n = 3)
#1.000000 0.004161 0.201570 - BH doesnt change
#BH 0.874400 0.004161 0.100785




#with brome data and no evenness - whole community
correct_BRAR <- c(0.1922, 0.3476, 0.02802, 0.6794)
p.adjust(correct_BRAR, method = "BH", n = 4)
#0.76880 1.00000 0.11208 1.00000 - nothing changes with BH
#BH 0.3844000 0.4634667 0.1120800 0.6794000

correct_BRTE <- c(0.03639, 0.22, 0.001718, 0.7106)
p.adjust(correct_BRTE, method = "BH", n = 4)
#0.145560 0.880000 0.006872 1.000000 - first becomes marginally sig with BH
#BH 0.0727800 0.2933333 0.0068720 0.7106000



#light and sm adjustments
correct_ab_BRAR <- c(0.000001185, 0.6136, 8.842E-07, 0.1795)
p.adjust(correct_ab_BRAR, method = "BH", n = 4)
#4.7400e-06 1.0000e+00 3.5368e-06 7.1800e-01
#BH 0.00000237 0.61360000 0.00000237 0.2393333

correct_ab_BRTE <- c(0.00003883, 0.2489, 2.28E-08, 0.222)
p.adjust(correct_ab_BRTE, method = "BH", n = 4)
#1.5532e-04 9.9560e-01 9.1200e-08 8.8800e-01
#BH 7.766e-05 2.489e-01 9.120e-08 2.489e-01
